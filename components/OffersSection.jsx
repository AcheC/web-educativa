...
export default function OffersSection() {
    ...
    return (
        <section
            id="offers"
            className= {`${dmSans.className} relative min-h-[580px] flex items-center text-white`}
            aria-labelledby="offers-heading"
        >
            {/* Background image (fill uses parent .relative) */}
            <Image src="/offers-bg.jpg" alt="Imagen Ofertas" fill className="object-cover" priority />
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-4 py-16">
                <h2 id="offers-heading" className={`text-center md:text-4xl font-bold text-white`}>Ofertas Académicas</h2>
                ...
                <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-20">
                    {CARD_ITEMS.map((item) => (
                        ...
                        >
                            ...
                            <h3 className="text-lg font-semibold text-[var(--color-text-card)]">{item.title}</h3>
                            <p className="mt-2 text-[var(--color-text-secondary)] text-sm">{item.description}</p>
